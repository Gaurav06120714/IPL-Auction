...
import { roomsTable, usersTable, squadTable, playersTable } from "../db/schema.js";
...
import {
  ...
  getResultsForRoom,
} from "../services/auction-engine.js";
...
router.post("/rooms", async (req, res) => {
  ...
  const [user] = await db
    .insert(usersTable)
    .values({ name: hostName, roomId, teamName, balance: 10000, squadCount: 0, isHost: true })
    .returning();

  res.status(201).json({
    roomId,
    status: "waiting",
    users: [{ id: user.id, name: user.name, teamName: user.teamName, balance: user.balance, squadCount: user.squadCount, isHost: user.isHost }],
  });
});
...
router.get("/rooms/:roomId", async (req, res) => {
  ...
  const room = await db.query.roomsTable.findFirst({ where: eq(roomsTable.id, params.data.roomId) });
  if (!room) { res.status(404).json({ error: "Room not found" }); return; }

  const users = await db.query.usersTable.findMany({ where: eq(usersTable.roomId, room.id) });

  res.json({
    roomId: room.id,
    status: room.status,
    users: users.map((u) => ({ id: u.id, name: u.name, teamName: u.teamName, balance: u.balance, squadCount: u.squadCount, isHost: u.isHost })),
  });
});
...
router.post("/rooms/:roomId/join", async (req, res) => {
  ...
  if (!room) { res.status(404).json({ error: "Room not found" }); return; }
  if (room.status !== "waiting") { res.status(400).json({ error: "Auction has already started" }); return; }

  const existingUsers = await db.query.usersTable.findMany({ where: eq(usersTable.roomId, roomId) });
  if (existingUsers.length >= 10) { res.status(400).json({ error: "Room is full (max 10 players)" }); return; }

  const teamTaken = existingUsers.some((u) => u.teamName === teamName);
  if (teamTaken) { res.status(400).json({ error: `Team ${teamName} is already taken` }); return; }

  const [user] = await db.insert(usersTable)
    .values({ name, roomId, teamName, balance: 10000, squadCount: 0, isHost: false })
    .returning();

  res.json({ userId: user.id, roomId, name: user.name, teamName: user.teamName, isHost: false });
});
...
router.post("/rooms/:roomId/start", async (req, res) => {
  ...
  const room = await db.query.roomsTable.findFirst({ where: eq(roomsTable.id, roomId) });
  if (!room) { res.status(404).json({ error: "Room not found" }); return; }
  if (room.status !== "waiting") { res.status(400).json({ error: "Auction already started" }); return; }

  const user = await db.query.usersTable.findFirst({
    where: and(eq(usersTable.id, userId), eq(usersTable.roomId, roomId)),
  });
  if (!user?.isHost) { res.status(403).json({ error: "Only the host can start the auction" }); return; }

  await startAuction(roomId);
  res.json({ success: true, message: "Auction started" });
});

// POST /api/rooms/:roomId/bid
...
// GET /api/rooms/:roomId/users/:userId/squad
router.get("/rooms/:roomId/users/:userId/squad", async (req, res) => {
  ...
  const squadEntries = await db
    .select({
      id: playersTable.id, name: playersTable.name, role: playersTable.role,
      country: playersTable.country, category: playersTable.category,
      basePrice: playersTable.basePrice, powerScore: playersTable.powerScore,
  ...
  res.json({ userId: user.id, name: user.name, teamName: user.teamName, balance: user.balance, squadCount: user.squadCount, squad: squadEntries, totalPowerScore });
});
