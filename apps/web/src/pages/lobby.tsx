...
import { useGetRoom, useStartAuction } from "@workspace/api-client-react";
...
export default function Lobby() {
  ...
  // Socket connection ensures we get real-time room updates
  ...
  const startAuction = useStartAuction({
    mutation: {
      onError: (error: any) => {
        toast({ title: "Cannot start", description: error.message || "Something went wrong", variant: "destructive" });
      }
    }
  });
  ...
  const handleStart = () => {
    if (session?.userId) {
      startAuction.mutate({ roomId, data: { userId: session.userId } });
    }
  };
          ...
          <div className="flex items-center justify-between mb-8 border-b border-white/10 pb-4">
            ...
            {session?.isHost && (
              <Button 
                ...
                className="bg-primary hover:bg-primary/90 text-primary-foreground font-bold text-lg px-8 shadow-[0_0_15px_rgba(245,158,11,0.3)] animate-pulse-ring"
              >
