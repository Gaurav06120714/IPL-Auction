...
import { useGetAuctionState, usePlaceBid, useGetUserSquad } from "@workspace/api-client-react";
...
export default function Auction() {
  ...
  const { data: state, isLoading } = useGetAuctionState(roomId, {
    query: { refetchInterval: false },
  });

  const { data: squadData } = useGetUserSquad(roomId, session?.userId || 0, {
    query: { enabled: !!session?.userId },
  });
  ...
  const handleBid = () => {
    ...
    placeBid.mutate({ roomId, data: { userId: session.userId, amount: nextBid } });
  };
