...
export default function Results() {
  ...
  useEffect(() => {
    ...
    const frame = () => {
      confetti({ particleCount: 5, angle: 60, spread: 55, origin: { x: 0 }, colors: ["#F59E0B", "#3B82F6"] });
      confetti({ particleCount: 5, angle: 120, spread: 55, origin: { x: 1 }, colors: ["#F59E0B", "#E11D48"] });
          ...
          {[1, 0, 2].map((rankIndex) => {
            ...
            return (
              ...
              >
                ...
                <Card className={`w-full ${heightClass} glass-panel border-t-4 p-6 flex flex-col items-center justify-start text-center relative overflow-hidden ${isWinner ? "border-t-yellow-400 bg-yellow-500/10" : rankIndex === 1 ? "border-t-slate-300 bg-slate-500/10" : "border-t-amber-700 bg-amber-900/10"}`}>
                  ...
                  <TeamBadge team={teamInfo} teamName={result.teamName} />
...
function TeamBadge({ team, teamName }: { team: any; teamName: string }) {
  if (!team) return <span className="text-sm text-white/60">{teamName}</span>;
  return <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${team.color} ${team.text}`}>{team.name}</span>;
}
